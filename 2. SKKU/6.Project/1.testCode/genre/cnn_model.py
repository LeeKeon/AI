...
#         self.genre_prob = tf.nn.softmax(tf.matmul(l3, w3) + b3)
        ...
        conv1 = self.conv(name = 'c1', inputs = self.x_image, shape=[3,3,3,32], s = 1, padding = 'SAME')
        conv1 = tf.nn.relu(conv1)
        conv1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
        conv2 = tf.nn.dropout(conv2, self.keep_prob)
        
        conv2 = self.conv(name = 'c2', inputs = conv1, shape=[3,3,32,64], s = 1, padding = 'SAME')
        conv2 = tf.nn.relu(conv2)
        conv2 = tf.nn.max_pool(conv2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
        conv2 = tf.nn.dropout(conv2, self.keep_prob)
        
        _, w, h, d = conv2.get_shape()
        length = int(w) * int(h) * int(d)
        w1 = self.get_var(name='w1', shape=[length, 128])
        b1 = self.get_var(name='b1', shape=[128])
        l1 = tf.reshape(conv2, [-1, length])
        l2 = tf.nn.relu(tf.matmul(l1, w1) + b1)
        l2 = tf.nn.dropout(l2, self.keep_prob1)
        
        w2 = self.get_var(name='w2', shape=[128, 23])
        b2 = self.get_var(name='b2', shape=[23])
        self.genre_prob = tf.nn.softmax(tf.matmul(l2, w2) + b2)
        #최종 결과
