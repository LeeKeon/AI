...
class movie_data(object):
    ...
    def get_test(self, batch_size=20):
        ...
        return self.x_img_test[pt: pt+batch_size], self.x_ids_test[pt: pt+batch_size], self.x_len_test[pt: pt+batch_size],self.y_test[pt: pt+batch_size]
    ...
    def init_data_test(self, files):
        ...
        for file in files:
            filename = file.split('.')[0] + '.txt'
            with open(self.plot_path + filename, "r", encoding="utf-8") as fin:
                summary_plots.append(fin.readline())

        for plots in summary_plots:
