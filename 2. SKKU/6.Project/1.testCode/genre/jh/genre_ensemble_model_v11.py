...
#         text_vec = tf.concat([fw_state.h, bw_state.h], 1)
        ...
        self.genre_prob_img = tf.contrib.layers.fully_connected(fc_l1, self.class_size, activation_fn=tf.nn.tanh)
        self.genre_prob_txt = tf.contrib.layers.fully_connected(lstm_out, self.class_size, activation_fn=tf.nn.tanh)
        p = tf.clip_by_value(tf.Variable(0.5, name = 'prob', dtype='float32'), 1e-20, 0.7)
        q = 1-p
        self.genre_prob = tf.nn.softmax(tf.add(self.genre_prob_img * p, self.genre_prob_txt * q))
