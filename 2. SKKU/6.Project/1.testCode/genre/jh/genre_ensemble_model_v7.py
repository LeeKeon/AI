...
class Model(object):
    ...
    def build_model(self):
        conv1 = self.conv(name = 'c1', inputs = self.x_image, shape=[5,5,3,64], s = 1, padding = 'SAME')
        ...
        conv4 = self.conv(name = 'c4', inputs = conv3, shape=[3,3,128,128], s = 1, padding = 'SAME')
        conv4 = tf.nn.relu(conv4)
        # conv3 = tf.nn.max_pool(conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='VALID')
        # conv3 = tf.nn.dropout(conv3, self.keep_prob)
        self.con4 = conv4

        x_image = tf.add(conv2, conv4)
#         print('x_image',x_image)
        _, w, h, d = x_image.get_shape()
        length = int(w) * int(h) * int(d)
        img_vector = tf.reshape(x_image, [-1, length])
        img_vector = tf.nn.dropout(img_vector, self.keep_prob)
        
        fc_l1 = tf.contrib.layers.fully_connected(img_vector, 128, activation_fn=tf.nn.relu,weights_regularizer=tf.contrib.layers.l2_regularizer(scale=0.2))
        # fc_l1= tf.nn.dropout(fc_l1, self.keep_prob)
...
#         embedding = tf.concat([self.x_emb, img_feature], axis=2)
        ...
        (forward_output, backward_output), _ = tf.nn.bidirectional_dynamic_rnn(
            fw_cell, bw_cell, inputs=self.x_emb, sequence_length=self.x_len, dtype=tf.float32,scope='BLSTM_1')


        outputs = tf.concat([forward_output, backward_output], axis=2)

        #Second BLSTM using the output of previous layer as an input.
        fw_cell2 = rnn.DropoutWrapper(rnn.BasicLSTMCell(self.hidden_dim), output_keep_prob=self.keep_prob)
        bw_cell2 = rnn.DropoutWrapper(rnn.BasicLSTMCell(self.hidden_dim), output_keep_prob=self.keep_prob)

        output, (fw_state, bw_state) = tf.nn.bidirectional_dynamic_rnn(
            fw_cell2, bw_cell2, inputs=outputs, sequence_length=self.x_len, dtype=tf.float32,scope='BLSTM_2')
        # (forward_output, backward_output), _ = tf.nn.bidirectional_dynamic_rnn(
        #     fw_cell, bw_cell, inputs=outputs, sequence_length=self.x_len, dtype=tf.float32,scope='BLSTM_2')
        # text_vec = tf.concat([forward_output, backward_output], axis=2)
...
#         lstm_out = tf.matmul(text_vec, out_W) + out_b
        ...
        lstm_out = tf.contrib.layers.fully_connected(text_vec, 128, activation_fn=tf.nn.tanh,weights_regularizer=tf.contrib.layers.l2_regularizer(scale=0.5))
#         text_vec = tf.concat([fw_state.h, bw_state.h], 1)
