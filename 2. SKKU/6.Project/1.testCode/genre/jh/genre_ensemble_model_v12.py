...
#         print('x_image',x_image)
        ...
        img_vector = tf.reshape(img_cell, [-1, length])
        # istate_fw = tf.reduce_mean(img_vector, axis=1, keep_dims=True)
        cnn_state = tf.contrib.layers.fully_connected(img_vector, self.hidden_dim, activation_fn=tf.nn.relu,weights_regularizer=tf.contrib.layers.l2_regularizer(scale=0.2))
        print(cnn_state.shape)
        # istate_fw = rnn.LSTMStateTuple(tf.concat([init_state, init_state], 1))

        istate_fw = rnn.LSTMStateTuple(cnn_state, cnn_state)
        # print(istate_fw.shape)
        ...
        output, (fw_state, bw_state) = tf.nn.bidirectional_dynamic_rnn(
            fw_cell, bw_cell, inputs=self.x_emb, sequence_length=self.x_len, initial_state_fw=istate_fw, initial_state_bw=istate_fw, dtype=tf.float32)
...
#         text_vec = tf.concat([fw_state.h, bw_state.h], 1)
        ...
        self.genre_prob_img = tf.contrib.layers.fully_connected(fc_l1, self.class_size, activation_fn=tf.nn.softmax)
        self.genre_prob_txt = tf.contrib.layers.fully_connected(lstm_out, self.class_size, activation_fn=tf.nn.softmax)
        p = tf.clip_by_value(tf.Variable(0.5, name = 'prob', dtype='float32'), 1e-20, 0.8)
        q = 1-p
        self.genre_prob = tf.add(self.genre_prob_img * p, self.genre_prob_txt * q)
