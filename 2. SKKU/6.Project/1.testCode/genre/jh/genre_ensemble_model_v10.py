...
#         text_vec = tf.concat([outputs_fw[-1], outputs_bw[-1]], axis=1)
...
#         text_vec = tf.concat([fw_state.h, bw_state.h], 1)
        ...
        p = tf.clip_by_value(tf.Variable(0.5, name = 'prob', dtype='float32'), 1e-20, 0.7)
        q = 1-p
    ...
    def save(self, sess, global_step=None):
        ...
        save_path = saver.save(sess, save_path="models/model10", global_step=global_step)
    ...
    def restore(self, sess):
        ...
        var_list = [var for var in tf.all_variables()]
        saver = tf.train.Saver(var_list)
        saver.restore(sess, "models/model10")
