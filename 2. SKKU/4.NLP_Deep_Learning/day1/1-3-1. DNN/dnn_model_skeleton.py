...
class Model(object):
    ...
    def build_model(self):
        ...
        d1=128
        W1 = self.get_var(name="weight1", shape=[self.emb_dim * self.num_k, d1])
        b1 = self.get_var(name="bias1", shape=[d1])
        layer_1 = self.leaky_relu(tf.matmul(self.x_emb_2d,W1)+b1)
        
        d2=256
        W2 = self.get_var(name="weight2", shape=[d1, d2])
        b2 = self.get_var(name="bias2", shape=[d2])
        layer_2 = self.leaky_relu(tf.matmul(layer_1,W2)+b2)
