...
class Model(object):
    ...
    def build_model(self):
        ...
        conv_l1 = self.leaky_relu(tf.nn.conv2d(x_emb_4d, filter=conv_f1, strides=[1, 1, 1, 1], padding='VALID')

        conv_f2 = self.get_var(name = 'conv_f2', shape=[3, 1, 256, 256])
        conv_l2 = self.leaky_relu(tf.nn.conv2d(conv_l1, filter=conv_f2, strides=[1, 1, 1, 1], padding='VALID')
        

        conv_f3 = self.get_var(name = 'conv_f3', shape=[3, 1, 256, 256])
        conv_l3 = self.leaky_relu(tf.nn.conv2d(conv_l2, filter=conv_f3, strides=[1, 1, 1, 1], padding='VALID')
