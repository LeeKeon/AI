...
class Data(object):
    ...
    def __init__(self, path, save_name, refresh_data=False,
        ...
        self.eos_token, self.eos_idx = "</s>", 0
        self.stt_token, self.stt_idx = "<s>", 1
        self.unk_token, self.unk_idx = "<unk>", 2
        ...
        if os.path.exists(self.save_name) and not refresh_data:
            ...
            # Val
            enc, dec, size = self.read_file("tst2012")
            self.text_enc.extend(enc)
            self.text_dec.extend(dec)
            self.val_size += size

            # Test
            enc, dec, size = self.read_file("tst2013")
            self.text_enc.extend(enc)
            self.text_dec.extend(dec)
            self.test_size += size
    ...
    def read_file(self, name="tst2012"):
    ...
    def build_data(self):
        ...
        for i in range(self.size):
            ...
            enc = self.text_enc[i].split()
            dec = self.text_dec[i].split()
