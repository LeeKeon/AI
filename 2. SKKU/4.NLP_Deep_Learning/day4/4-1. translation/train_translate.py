...
data = Data(path='./dataset/vi2en', save_name='vi2en.data', refresh_data=True,
...
def sample_test():
    idx, enc, dec, enc_len, dec_len = data.get_batch('test', 1)
    output_token = sess.run(model.output_token, feed_dict={model.x: enc, model.x_len: enc_len})
    out_line = " ".join([data.idx2w_dec[o] for o in output_token[0]])
    true_line = " ".join([data.idx2w_dec[o] for o in dec[0]])
    print(" - true: {}\n --> output: {}".format(true_line, out_line))
